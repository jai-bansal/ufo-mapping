...
shinyServer(function(input, output)
  ...
  output$plot1 = renderPlot({
    ...
    # CREATE MAP:
      ...
      # Create reactive map of United States:
      # The specified longitude/latitude coordinates are roughly the geographic center
      # of the United States:
      map = reactive({
                      ggmap(get_googlemap(center = c(longitude = -96.35, latitude = 39.5), 
                                          zoom = 4, maptype = 'terrain'))
                    })
      
      # Create reactive version of lower date bound:
      lower_bound = reactive({input$dates[1]})
      ...
      rel_data = ufo_data[(date >= lower_bound()) & (date < new_upper_bound)]
